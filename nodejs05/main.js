...
function getQuadrant(signals) {
  ...
  for (const signal of signals) {
    ...
    secuence.push(quadrants[quadrant]);
  }

  return secuence.join("");
}

function getCycles(secuence) {
  const cycles = secuence.match(/(NESO|OSEN|ESON|SENO|SONE|ENOS|ONES|NOSE)/g);
  if (cycles) {
...
function calculatePercentage(signals) {
  ...
  for (let quadrant in quadrants) {
    quadrants[quadrant] = (
      (quadrants[quadrant] / signals.length) *
      100
    ).toFixed(2);
  }
...
function main() {
  ...
  const sequence = getQuadrant(signals);
  ...
  document.addEventListener("DOMContentLoaded", (event) => {
    ...
    document.getElementById("porcentaje").innerText =
      JSON.stringify(quadrantPercentages);
  });
}
main();
